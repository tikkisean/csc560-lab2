...
func (a *Aggregator) Descriptor() *TupleDesc {
	...
	if a.groupByFields != nil {
		tds := make([]FieldType, len(a.groupByFields))
		for i, f := range a.groupByFields {
			tds[i] = f.GetExprType()
		}
		newTd = &TupleDesc{tds}
	}
	for _, nas := range a.newAggState {
		nasTd := nas.GetTupleDesc()
		if newTd == nil {
			newTd = nasTd
		} else {
			newTd = newTd.merge(nasTd)
		}
	}
	return newTd
}
...
func extractGroupByKeyTuple(a *Aggregator, t *Tuple) (*Tuple, error) {
	outVals := make([]DBValue, len(a.groupByFields))
	outDesc := make([]FieldType, len(a.groupByFields))
	var err error
	for i := 0; i < len(a.groupByFields); i++ {
		outVals[i], err = a.groupByFields[i].EvalExpr(t)
		if err != nil {
			return nil, err
		}
		outDesc[i] = a.groupByFields[i].GetExprType()
	}
	outTup := &Tuple{TupleDesc{outDesc}, outVals, nil}
	return outTup, nil
}
...
func addTupleToGrpAggState(a *Aggregator, t *Tuple, grpAggState *[]AggState) {
	for i := 0; i < len(a.newAggState); i++ {
		as := (*grpAggState)[i]
		if as == nil {
			as = a.newAggState[i].Copy() // this is copying a newly initialized AggState
			(*grpAggState)[i] = as
		}
...
func getFinalizedTuplesIterator(a *Aggregator, groupByList []*Tuple, aggState map[any]*[]AggState) func() (*Tuple, error) {
	...
	return func() (*Tuple, error) {
		...
		gby := groupByList[curGbyTuple]
		key := gby.tupleKey()
		var tup *Tuple
		for i := 0; i < len(a.newAggState); i++ {
			newTup := (*aggState[key])[i].Finalize()
			tup = joinTuples(tup, newTup)
		}
		rett := joinTuples(gby, tup)
		curGbyTuple++
		return rett, nil
	}
}
